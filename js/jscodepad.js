...
!function(){
  ...
  window.addEventListener("load", function(){
      ...
      [].forEach.call( document.querySelectorAll(".JSCodePad"), function(domCodePad) {
		  ...
		  consoleDom.addEventListener("dblclick", function(e) {              
              consoleDom.innerHTML="";
          }, false);
      });
...
//********************************************************	  	  
//******** Dynamic Load Sample into PlayGround XE.NET ****
if ($("jsPlayground")) {  
  var _jsplayground = ace.edit("jsPlayground"); //editor del PlayGround
	
  window.XE_LoadSample = function (url){		
	hideTestGround(false);
	if (url) {
		var req = false
		...
		req.onreadystatechange=function(){
			if (req.readyState == 4 && (req.status==200 || window.location.href.indexOf("http")==-1)) {
				_jsplayground.setValue(req.responseText,-1);
			}
		}
		...
		_jsplayground.setValue('');
	} 
  };
  
  if ($("chooseSamples")) {
	  $("chooseSamples").addEventListener("change", function(e) {  
		  XE_LoadSample($("chooseSamples").value);
	  }, false);
  }
}
//********************************************************
